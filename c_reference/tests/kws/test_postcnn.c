...
int main(){
    ...
    DSCNN_LR_Point_Depth(cnn2_out, INPUT, in_T, POST_CNN_I_F, CNN2_BNORM_MEAN, CNN2_BNORM_VAR,
    ...
    out_T = in_T - DEPTH_FILT + 2*(DEPTH_FILT>>1) + 1; // Depth pad = 2 for kernel size =5. SAME PAD
    out_T = out_T - POST_CNN_POOL + 2*(0) + 1; // Pool pad = none/0
    // float* cnn5_out = (float*)malloc(out_T * POST_CNN_I_F * sizeof(float));
