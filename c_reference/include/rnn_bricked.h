...
-> Single-directional Computation
  While using the bricked fastgrnn layers, the two following constraints need to be adhered to
  1) in_time % hop == 0
  2) fwd_window % hop == 0 and bwd_window % hop == 0

  Violation of the above two constraints (1 & 2), will cause segmentation faults
  The layers first compute all the Wx steps and then compute all the windows parallelly
  Hence, the constraints (1 & 2) need to be satisfied
...
-> Bi-directional Computation
  ...
  B) An offset of rnn_hidden would need to be given to the output_signal pointer during the backward sssfunction call
  ...
  If the first (last) bricks are not sampled, the first few (last few) time steps would be missing in the forward (backward) results. 
  If the offset is not passed during the backward function call, the backward pass will overwrite the forward result (bi-directional case)
*/
